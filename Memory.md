...
- **Current Phase:** Phase 12 — Polish & Hardening
- **Status:** Complete. Loading/error/not-found states across the dashboard, a full permission audit (no gaps found), pagination on every previously-unbounded list, a `/notifications` page, and the codebase's first automated route-handler tests (12 files, 36 tests, against a real DB). See this session's log.
...
- **`CRON_SECRET` has no scheduler pointed at it yet (Phase 6).** `POST /api/jobs/recalculate-workload` exists and is guarded, but nothing external calls it on a schedule — the same open hosting decision above needs settling first (Vercel Cron, GitHub Actions, etc., once a host exists).
