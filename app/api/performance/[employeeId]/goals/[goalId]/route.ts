...
/**
 * PATCH /api/performance/[employeeId]/goals/[goalId] — mark a goal Achieved
 * or Missed (Phases.md Phase 8).
 *
...
export async function PATCH(
  request: NextRequest,
  context: RouteContext<"/api/performance/[employeeId]/goals/[goalId]">
) {
  const actor = await getActor();
  if (!actor) return unauthorized();
  if (actor.accountType !== "company") return forbidden();

  const { employeeId, goalId } = await context.params;
  ...
  try {
    ...
    return serverError(
      {
        route: "PATCH /api/performance/[employeeId]/goals/[goalId]",
